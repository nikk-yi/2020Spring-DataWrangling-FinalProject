...
top10
```
  UK lacks data for recovered case. 
...
usa_covid19 <- as.data.frame(usa_covid19) %>%
  ...
  select(State, TotalCases, TotalDeaths) %>%
...
usa_covid19 %>% 
  ...
  state_choropleth(title = "Total cases of Covid-19 in US by State",  num_colors = 9)

usa_covid19 %>% 
  dplyr::rename(value = Death.rate) %>% 
  state_choropleth(title = "Death Rate of Covid-19 in US by State",  num_colors = 9)

# Population need in hospital
usa_covid19 %>% 
  dplyr::rename(value = CriticalCases) %>% 
  state_choropleth(title = "Total Critical Cases of Covid19 in US by State",  num_colors = 9)
```
...
hospital_stats <- as.data.frame(hospital_stats)[-57,] %>%
  separate(State, c("abb", "State"), sep = "- ") %>%
  select(State, StaffedBeds, PatientDays) %>%
...
elderly_pop <- elderly_pop[-1,] %>%
  select(State, TotalPop.k, ElderlyPop.k, Elderly.p) %>%
  ...
  mutate(TotalPop.k = as.numeric(TotalPop.k),
         ...
         Elderly.p = as.numeric(Elderly.p)/100,
         region = tolower(State)) 
...
elderly_pop %>%
  ...
  state_choropleth(title = "Elderly Population in US by State",  num_colors = 1)
```
...
usa_data <- usa_covid19 %>%
  ...
  arrange(desc(Infect.rate_k, TotalCases))
...
usa_data %>% 
  ...
  state_choropleth(title = "Infected Rate of Covid19 in US by State",  num_colors = 9)
```

```{r}
# Poisson regression
lm <- glm(TotalDeaths ~  StaffedBeds + TotalPop.k + ElderlyPop.k + Infect.rate_k, 
           data = usa_data, family = poisson(link="log"))
summary(lm)

lm2 <- glm(TotalDeaths ~  StaffedBeds + TotalPop.k + ElderlyPop.k + TotalCases, 
           data = usa_data, family = poisson(link="log"))
summary(lm2)
```
...
sum(usa_data$TotalCases)/2

lm_totalcase <- usa_data %>%
  mutate(pandamic.center = ifelse(TotalCases >= 50000, 1, 0)) %>%
...
lm_infectrate <- usa_data %>%
  mutate(pandamic.center = ifelse(Infect.rate_k >= 6, 1, 0)) %>%
  select(State, TotalCases, TotalDeaths, Death.rate, Infect.rate_k, StaffedBeds, TotalPop.k, ElderlyPop.k, pandamic.center) %>%
  group_by(pandamic.center) %>%
  nest() %>%
  mutate(lm_fit= map(data, deathlm))
